...
def main(args):
    ...
    # ADDED: TensorBoard logging for comparison
    agent = PPO(
        "MultiInputPolicy",
        envs,
        n_steps=N_STEPS,
        verbose=2,
        tensorboard_log=f"./logs/{args.env}_baseline"  # Save logs for TensorBoard
    )

    # ADDED: Checkpoint callback to save models
    checkpoint_callback = CheckpointCallback(
        save_freq=25000 // N_ENVS,  # Adjusted for parallel envs
        save_path=f"./models/{args.env}_baseline",
    ...
    try:
        agent.learn(
            ...
            callback=checkpoint_callback,  # ADDED: Save checkpoints
            progress_bar=True,
        )
    except ImportError:
    ...
    final_path = f"./models/{args.env}_baseline/final_model"
